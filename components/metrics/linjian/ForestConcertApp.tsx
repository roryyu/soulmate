  ...
  return (
    <main className="player-screen" ref={screenRef}>
      ...
      <section className="player-content">
        ...
        <button className="finish-button" type="button" onClick={() => onFinish(musicurl)}>
          <RotateCcw size={17} />
          完成本次体验
        </button>
...
/** 主应用：管理整个流程状态机（/metrics 嵌入版，不持久化进度，每次进入重新开始） */
export default function ForestConcertApp({
  onNext,
}: {
  /** 量表答题结束（transition 阶段）时触发，把答题数据包交给外层进入脑机步骤 */
  onNext: (packet: ResultPacket | null) => void
}) {
  ...
  /** 量表答题结束（transition 阶段）：把答题数据包交给外层，进入脑机步骤 */
  useEffect(() => {
    if (progress.phase === 'transition') onNext(packet)
  }, [progress.phase, packet, onNext])

  /** 完成体验（player 阶段，当前流程已不可达，保留备用） */
  const handleFinish = () => {
    onNext(packet)
  }
  ...
  if (rewardIndex != null) {
    ...
    // 切换过场不再播放：答题数据包经上方 useEffect 交回外层，进入脑机步骤
    screen = null
