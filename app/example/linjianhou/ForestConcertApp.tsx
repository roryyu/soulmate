...
function QuestionScreen({
  ...
  const questionAudioSrc = AUDIO_FILES.find((item) => item.id === question.id)?.src
  ...
  // 每打开一次新题目，题目音频自动播放一次（声音关闭时不播放）
  useEffect(() => {
    const audio = questionAudioRef.current
    if (!audio || !questionAudioSrc || !audioEnabled) return
    audio.currentTime = 0
    audio.play().catch(() => {})
  }, [question.id, questionAudioSrc])

  // 全局声音关闭时停止题目音频
  useEffect(() => {
    if (!audioEnabled) questionAudioRef.current?.pause()
  }, [audioEnabled])

  const toggleQuestionAudio = () => {
    const audio = questionAudioRef.current
    if (!audio || !questionAudioSrc) return
    if (questionAudioPlaying) {
      audio.pause()
      return
    }
    if (!audioEnabled) onAudioChange(true)
      ...
      <div className="question-content" ref={contentRef}>
        ...
        <section className="animal-dialogue">
          ...
          <div className="speech-bubble">
            ...
            <div className="question-audio-row">
              <button
                ...
                aria-label={questionAudioPlaying ? '暂停播放题目语音' : '播放题目语音'}
                title={questionAudioPlaying ? '暂停播放题目语音' : '播放题目语音'}
              >
                {questionAudioPlaying ? <Pause size={22} /> : <Play size={22} />}
              </button>
              <h1>{question.text}</h1>
            </div>
          </div>
          <audio
            ...
            onEnded={() => setQuestionAudioPlaying(false)}
          />
        </section>
